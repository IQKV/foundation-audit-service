...
/**
 * Provisions the audit service schema for a newly created tenant.
 *
 * <p>Listens on {@code iqkv.audit.tenant.provisioning} for {@code tenant.created} events
 * published by the IAM service and runs Liquibase tenant migrations against the audit database.
...
public class TenantProvisioningConsumer {
  ...
  public void handleTenantProvisioning(final Map<String, Object> event) {
    ...
    try {
      ...
      log.info("Audit schema provisioning succeeded: tenantKey={}", tenantKey);
    } catch (final Exception e) {
      log.error("Audit schema provisioning failed: tenantKey={}", tenantKey, e);
      // Rethrow so the message is nacked and routed to the DLQ for redelivery / inspection.
      throw new RuntimeException("Audit schema provisioning failed for tenantKey=" + tenantKey, e);
    }
  }
}
